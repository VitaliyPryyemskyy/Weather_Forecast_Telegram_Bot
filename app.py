...
def login():
    if request.method == "POST":
        ...
        with open("database.txt", "r") as file:
            for line in file:
                if first_name in line and last_name in line:
                    return f"welcome {first_name} {render_template('index.html')}"
                elif first_name == False and last_name == False:
                    return f"Enter name and lastname{render_template('login.html')}"
            return f" Invalid username {render_template('login.html')}"
    else:
